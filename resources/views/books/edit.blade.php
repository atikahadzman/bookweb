        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="flex flex-col md:flex-row md:space-x-6">
                    ...
                    <div class="md:w-1/4 flex-shrink-0 mb-6 md:mb-0 flex justify-center md:justify-start">
                        ...
                        @else
                            <div class="w-40 h-60 bg-gray-200 flex items-center justify-center rounded-lg">
                    ...
                    <div class="md:w-3/4">
                        <form action="{{ Auth::user()->role === 'seller' 
                            ? route('books.update', $books->id) 
                            : route('books.status', $books->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="mt-4">
                                ...
                                <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="old('title', $books->title)" />
                                <x-input-error class="mt-2" :messages="$errors->get('title')" />
                            </div>

                            <div class="mt-4">
                                <x-input-label for="author" :value="__('Author')" />
                                <x-text-input id="author" name="author" type="text" class="mt-1 block w-full" :value="old('author', $books->author)" />
                                <x-input-error class="mt-2" :messages="$errors->get('author')" />
                            </div>

                            <div class="mt-4">
                                <x-input-label for="isbn" :value="__('ISBN')" />
                                <x-text-input id="isbn" name="isbn" type="text" class="mt-1 block w-full" :value="old('isbn', $books->isbn)" readonly/>
                           ...
                           <div class="mt-4">
                                ...
                                <textarea id="description" name="description"
                                    class="mt-1 block w-full h-48 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-gray-900">
                                    {{ old('description', $books->description ?? '') }}
                                </textarea>
                            ...
                            <div class="mt-4">
                                <x-primary-button>
                                    {{ Auth::user()->role === 'seller' ? __('Update') : __('Approve') }}
                                </x-primary-button>
                            </div>
                        </form>
                    </div>
